package com.xtremepixel.moviemvvm.upcoming.presentation.viewmodel
...
import com.xtremepixel.moviemvvm.upcoming.domain.GetUpcomingMovies
import com.xtremepixel.moviemvvm.upcoming.presentation.UpcomingMovieState
import com.xtremepixel.moviemvvm.upcoming.presentation.UpcomingMoviesEvent
...
class UpcomingMovieViewModel @Inject constructor(
    ...
    companion object {
        ...
        val state: LiveData<UpcomingMovieState> get() = _state
        var isLoadingMoreMovie: Boolean = false
        var isLastPage = false

        private val _state = MutableLiveData<UpcomingMovieState>()
        private var currentPage = 0

        init {
            _state.value = UpcomingMovieState()
            loadMovies()
        }

        fun onEvent(event: UpcomingMoviesEvent) {
            when(event) {
                is UpcomingMoviesEvent.RequestInitialMoviesList -> loadMovies()
                is UpcomingMoviesEvent.RequestMoreMoviess -> loadNextMoviePage()
            }
        }
